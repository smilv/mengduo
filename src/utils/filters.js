 ...
 * 字数超出...
 */
Vue.filter("interceptedWords", (value, separator) => {
    if (value && value.length > separator) {
        value = value.substring(0, separator - 1) + "...";
    }
    return value;
});
/**
 * 加密
 * type tel电话格式  185****8447
 * format 加密格式
 */
Vue.filter("encrypt", (value, type, format) => {
    if (type == "tel" && value.length == 11) {
        let first = value.toString().slice(0, 3);
        let last = value.toString().slice(-4);
        return first + format + format + format + format + last;
    }
});
 ...
 * 数字超出999显示1.0w
 */
Vue.filter("convertedDigitUnit", value => {
